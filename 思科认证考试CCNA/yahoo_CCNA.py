...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into Tokyo_CCNA (salary,type,link,job_name) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for PageNum in range(1,101):
        url = 'https://job.yahoo.co.jp/jobs/?&keyword=CCNA&l=%E6%9D%B1%E4%BA%AC%E9%83%BD&side=1&page='+str(PageNum)+'&ssid=80210ac4-2a30-4edc-9aa0-9b10c983d441'
        html = call_page(url)
