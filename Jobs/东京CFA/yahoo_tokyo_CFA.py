...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into Tokyo_CFA (salary,type,link,job_name) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for PageNum in range(1,101):
        url = 'https://job.yahoo.co.jp/jobs/?&keyword=CFA&l=東京都&page='+str(PageNum)+'&ssid=25158ae9-0705-4dd3-85a3-30a1930eb123&comp=1'
        html = call_page(url)
