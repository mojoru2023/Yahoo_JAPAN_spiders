...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into Tokyo_fundm1 (salary,type,link,job_name) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for PageNum in range(1,101):
        url = 'https://job.yahoo.co.jp/jobs/?&keyword=ファンドマネージャー&l=東京都&page='+str(PageNum)+'&ssid=e03d6ba5-609b-4ad4-802b-b7d69e9e0ed9&comp=1'
        html = call_page(url)
