...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into Tokyo_fundm2 (salary,type,link,job_name) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for PageNum in range(1,101):
        url = 'https://job.yahoo.co.jp/jobs/?&keyword=go&l=%E6%9D%B1%E4%BA%AC%E9%83%BD&page='+str(PageNum)+'&ssid=4ff6454f-40df-4ae4-aca4-9b367366e463&comp=1'
        html = call_page(url)
