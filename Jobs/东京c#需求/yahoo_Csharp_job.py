...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into Tokyo_Csharp (salary,type,link,job_name) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for PageNum in range(22,101):
        url = 'https://job.yahoo.co.jp/jobs/?&keyword=c%23&l=%E6%9D%B1%E4%BA%AC%E9%83%BD&side=1&page='+str(PageNum)+'&ssid=fc8f9647-7c93-402c-be74-fecfba6252c6'
        html = call_page(url)
