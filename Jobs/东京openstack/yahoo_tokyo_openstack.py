...
def insertDB(content):
    ...
    try:
        cursor.executemany('insert into Tokyo_openstack (salary,type,link,job_name) values (%s,%s,%s,%s)', content)
        connection.commit()
        connection.close()
        print('向MySQL中添加数据成功！')
    except TypeError :
...
if __name__ == '__main__':
    for PageNum in range(1,101):
        url = 'https://job.yahoo.co.jp/jobs/?&keyword=openstack&l=東京都&page='+str(PageNum)+'&ssid=1ee2fb59-2b03-4181-aaa7-c5b70c6b1376&comp=1'
        html = call_page(url)
